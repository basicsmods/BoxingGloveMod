...
- (Done; got rid of reload entirely.) Only reload lua when applying settings if one of our settings was changed.
...
- (Added "upgrades" instead) Do something about the repair side of the mod.
- (Done) Make it so that the boxing helmet can be worn with a mask.
- Add option to have it drop from loot places instead of zombies.
- (Done) Maybe instead of them dropping, they are craftable? Or maybe only the tier 1s drop and then you can use X amount of them to make a tier 2. And then Y amount of tier 2 to make a tier 3. And no repairs?
- Add different colours of gloves (modify the texture png; white, pink, etc.)
- Give the player some kind of bonus if they are wearing the shorts and helmet as well.
- Let the gloves be upgraded even if they are in your backpack.

(Unrelated to this mod)
- Add hotkey to cycle through containers without needing to mouse over the window.
- Add a feature (maybe a button or just a setting) for susceptible to make the widget click-through. (Also make the widget's default position in the middle of the screen.)